<template>
  <div class="navbar">
    <div class="left-side">
      <a-space :style="{ cursor: 'pointer' }" @click="handleHome">
        <img
          alt="logo"
      ...
      <div v-if="!isHomePage">
        ...
        <a-space :style="{ color: theme === 'dark' ? '#ffffff' : '#000000' }">
          ...
          <a-select
            ...
            allow-search
          >
            <a-option v-for="(value, index) in namespaces" :key="index">
              {{ value }}
            </a-option>
          </a-select>
        </a-space>
...
<script lang="ts" setup>
  ...
  */
  ...
  watch(curNamespace, () => {
    clusterStore.curNamespace = curNamespace.value;
    clusterStore.save();
  });
  ...
  onMounted(() => {
    if (!isHomePage.value) {
      curNamespace.value = clusterStore.curNamespace;
      fetchClusterNamespace();
    }
  });
</script>
